...
public class SMSMaker extends NotificationMaker{//needs modification
    SMSMaker(QueueHandler queueHandler, TemplateMaker maker){
    ...
    public Notification makeNotification(User user, ArrayList<String> placeholders){
        ...
        Notification newNotification = new SMS(content, "created", user.getUserID(), user.getPhoneNumber(), templateMaker.getTemplate().getTemplateName());
